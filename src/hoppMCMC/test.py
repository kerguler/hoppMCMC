## Following code demostrates the use of hoppMCMC algorithm
## for jumping between the two basins of attraction of 100*(x^2-1)^2

from __init__ import hoppMCMC
import numpy
import pylab

## The following is the scoring function to be optimised
def fitness(x):
    return 100.0*pow(pow(x[0],2)-1.0,2)

## The following will run the hoppMCMC algorithm
results = hoppMCMC(fitness,    # define the score function
                   ...
                   model_comp = 1,     # temperature for accepting hopp-steps
...
for n in range(len(results.parmats)):
    pylab.plot(numpy.repeat(n,3),results.parmats[n][:,1],'o',c=["black","white"][n%2])
pylab.show()
